...
class ShareHiddenLayer_supp(object):
    def __init__(self, rng, input_source, input_supp, input_target, n_in, n_out,
                 W=None, b=None, activation=T.nnet.sigmoid, name='', dropout_rates=0):
        ...
        if dropout_rates>0:
            self.HL_source.output = _dropsout(rng, self.HL_source.output, p=dropout_rates)
            # self.HL_supp.output = _dropsout(rng, self.HL_supp.output, p=dropout_rates)
            self.HL_target.output = _dropsout(rng, self.HL_target.output, p=dropout_rates)

            self.output_source = self.HL_source.output
            # self.output_supp = self.HL_supp.output
...
class GaussianSampleLayer(object):
    def __init__(self, mu, log_sigma, n_in, batch_size):
        ...
        :param log_sigma: a symbolic tensor of shape (n_batch, n_in), means the log-variance(log_sigma). 
                          here using diagonal variance, so a cache_dataset has a row variance.
        '''
        seed = 42
        '''
...
class NN_Block_supp:
    def __init__(self, rng, input_source, input_supp, input_target, struct, params=None, name='', drop = None):
        ...
        tmp = ShareHiddenLayer_supp(
            rng=rng,
            input_source=input_source,
            input_supp=input_supp,
        ...
        self.learning_rate.append(
            theano.shared(value=np.float32(struct.learning_rate[0]), name=layer_name + '_lr', borrow=True))
        ...
        for i in range(l - 1):
            ...
            tmp = ShareHiddenLayer_supp(
                ...
                input_supp=self.Layer_group[-1].output_supp,
            ...
            self.learning_rate.append(
                theano.shared(value=np.float32(struct.learning_rate[j]), name=layer_name + '_lr', borrow=True))
            self.decay.append(struct.decay[j])

        self.output_source = self.Layer_group[-1].output_source
        self.output_supp = self.Layer_group[-1].output_supp
...
class Gaussian_MLP_supp:
    def __init__(self, rng, input_source, input_supp, input_target, struct, batch_size, params=None, name=''):
        if params == None:
            params = [None, None, None]

        # share_block
        self.share_block = NN_Block_supp(
            rng=rng,
            input_source=input_source,
            input_supp=input_supp,
            input_target=input_target,
        ...
        # mu output block
        self.mu_block = NN_Block_supp(
            rng=rng,
            input_source=self.share_block.output_source,
            input_supp=self.share_block.output_supp,
        ...
        # sigma output block
        self.sigma_block = NN_Block_supp(
            rng=rng,
            input_source=self.share_block.output_source,
            input_supp=self.share_block.output_supp,
            input_target=self.share_block.output_target,
        ...
        self.P_mu = self.mu_block.output_supp
        self.P_log_sigma = self.sigma_block.output_supp
        self.P_sigma = T.exp(self.P_log_sigma)
        ...
        self.GSL_supp = GaussianSampleLayer(
            mu=self.P_mu,
            log_sigma=self.P_log_sigma,
            n_in=struct.mu.layer_dim[-1],
            batch_size=batch_size[2]
        )

        self.S_output = self.GSL_source.output
        self.P_output = self.GSL_supp.output
